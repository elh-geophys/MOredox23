...
r_postCr_MC_1 = max(r_preCr_MC_1 - 0.35/8.1, 0);
...
r_postCr_MC_2 = max(r_preCr_MC_2 - 0.35/8.1, 0);

r_preCr_limit = 0.35/8.1;       %zero limit for preCr

r_postCr_range = [0.02 0.06];
r_preCr_range = r_postCr_range + 0.35/8.1;
...
ylabel('Fe^{3+}/\SigmaFe')
ylim([0 0.18])
yticks([0 0.03 0.06 0.09 0.12 0.15 0.18])
xlim([0 100])
text(3, 0.172, 'a', 'FontSize', 20, 'FontWeight', 'bold')
ax = gca;
ax.Layer = 'top';

axes('Position', [0.265 0.72 0.07 0.17]);
box on
...
yticks([0 0.03 0.06 0.09 0.12 0.15 0.18])
xlim([0 100])
text(3, 0.172, 'b', 'FontSize', 20, 'FontWeight', 'bold')
ax = gca;
ax.Layer = 'top';

axes('Position', [00.545 0.175 0.07 0.17]);
box on
...
plot(r_preCr_MC_1_wr0, r_postCr_MC_1_wr0, '-', 'Color', colors_red(1,:), "LineWidth", 2)
