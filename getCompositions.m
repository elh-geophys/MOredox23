...
hold on
box on
plot(Accr_model, OxiWts(5,:), 'r.', 'MarkerSize', 8)
...
if model == 5
    ...
    for i = 1:length(FeO_imp)
        if FeO_imp(i) > 0 && Accr_model(i+1) <= 1.0
            FeRatio_imp(i) = calcFeRatio_byIW(T_early, P_early, dIW_imp_long(i), PV_term_early, OxiWts_imp(:,i), OxiMolW_byM);
        elseif FeO_imp(i) > 0 && Accr_model(i+1) > 1.0
            FeRatio_imp(i) = calcFeRatio_byIW(T_Elike, P_Elike, dIW_imp_long(i), PV_term_Elike, OxiWts_imp(:,i), OxiMolW_byM);
        end
    end
    Accr_model = 0.5*Accr_model;
else
    for i = 1:length(FeO_imp)
        if FeO_imp(i) > 0 && Accr_model(i+1) < 0.67
            FeRatio_imp(i) = calcFeRatio_byIW(T_early, P_early, dIW_imp_long(i), PV_term_early, OxiWts_imp(:,i), OxiMolW_byM);
        elseif FeO_imp(i) > 0 && Accr_model(i+1) < 1.0
            FeRatio_imp(i) = calcFeRatio_byIW(T_late, P_late, dIW_imp_long(i), PV_term_late, OxiWts_imp(:,i), OxiMolW_byM);
        end
    end
end
