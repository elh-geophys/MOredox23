...
%parameterizing Rubie+2011 Fig 4b using data from Table S3a.
...
hold on
box on
plot(Accr_model, OxiWts(5,:), 'bo', 'MarkerSize', 8)
plot(F_temp, OxiWts_temp(5,:), 'k-')
xlim([0 1])
...
% FeO_Mol_E = zeros(1, length(FeO_E));
% FeO_Mol_test = zeros(1, length(FeO_E));
...
dIW_E_simple = 2*log10(FeO_Mol_E/0.79);
dIW_E_simple(isinf(dIW_E_simple)) = 0;
dIW_E_long = 2*log10((1.148*FeO_Mol_E + 1.319*FeO_Mol_E.^2)/0.78);     %with Rubie+11 param to X for wustite
...
FeO_imp = FeO_added./M_m_imp*100;             %[percent %] FeO content of the impactor mantle
...
OxiWts_late = zeros(length(OxiWts_late_temp), length(FeO_imp))+OxiWts_late_temp;
...
dIW_imp_simple = 2*log10(FeO_Mol_imp/0.78);
dIW_imp_simple(isinf(dIW_imp_simple)) = 0;
dIW_imp_long = 2*log10((1.148*FeO_Mol_imp + 1.319*FeO_Mol_imp.^2)/0.78);     %with Rubie+11 param to X for wustite
