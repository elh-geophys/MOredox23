...
%           compSheet   file data sheet for mole weights, Deng20 or Armstrong19
%
% OUTPUTS:  FeRatio [] Fe3+/sumFe as a function of P, T, and dIW
%

function [FeRatio,dIW] = calcFeRatio(T, P, F, PV_term, CompEarly_data, CompLate_data)

    % dIW value for Earth, see Rubie+2011, Supp. Table 3a (approximated as
    % linear gradient)
    dIW = 3*F-5;
    ...
    OxiWts_early = CompEarly_data(:,2);
    OxiMolW = CompEarly_data(:,3);

    %data = readmatrix('\db\MoleWeights.xlsx', 'Sheet', compSheet_late);
    OxiWts_late = CompLate_data(:,2);

    slope = OxiWts_late - OxiWts_early;
    OxiWts = slope.*F + OxiWts_early;
